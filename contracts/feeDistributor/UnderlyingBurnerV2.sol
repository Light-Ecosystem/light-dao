...
contract UnderlyingBurnerV2 is Ownable2StepUpgradeable, PausableUpgradeable {
    ...
    mapping(address => bool) public hasOperatorRole;
    modifier onlyOperatorRole() {
        require(hasOperatorRole[msg.sender], "account is missing operator role");
        _;
    }
    ...
    function burn(IERC20Upgradeable token, uint amount, uint amountOutMin) external onlyOperatorRole {
    ...
    function isOperator(address _operator) external view returns (bool) {
        return hasOperatorRole[_operator];
    }

    function addOperator(address _operator) external onlyOwner {
        require(_operator != address(0), "Zero address not valid");
        hasOperatorRole[_operator] = true;
    }

    function removeOperator(address _operator) external onlyOwner {
        require(_operator != address(0), "Zero address not valid");
        hasOperatorRole[_operator] = false;
    }
}
