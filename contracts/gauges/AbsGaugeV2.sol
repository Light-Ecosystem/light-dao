...
abstract contract AbsGaugeV2 is Ownable2Step {
    ...
    function _checkpoint(address _addr) internal {
        ...
        uint256 _weekTimeStamp = LibTime.timesRoundedByWeek(block.timestamp);
        if (!checkedGauge[_weekTimeStamp]) {
            checkedGauge[_weekTimeStamp] = true;
            controller.checkpointGauge(address(this));
        }
        ...
        if (block.timestamp > _st._periodTime) {
            ...
            for (uint256 i; i < 500; i++) {
                ...
                uint256 _preWeekTimeRound = LibTime.timesRoundedByWeek(_prevWeekTime);
                uint256 _w = historyGaugeRelativeWeight[_preWeekTimeRound];
                if (_w == 0) {
                    _w = controller.gaugeRelativeWeight(address(this), _preWeekTimeRound);
