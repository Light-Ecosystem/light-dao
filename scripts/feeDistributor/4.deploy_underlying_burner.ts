...
async function main() {
    ...
    let gaugeFeeDistributor = FileUtils.getContractAddress(Constants.GaugeFeeDistributor);
    let emergencyReturn = ethers.constants.AddressZero;

    const UnderlyingBurner = await ethers.getContractFactory("UnderlyingBurner");
    const underlyingBurner = await upgrades.deployProxy(UnderlyingBurner, [hopeToken, feeDistributor, gaugeFeeDistributor, emergencyReturn]);
