...
describe("ConnetVaultOfStHOPE", function () {
  ...
  async function deployOneYearLockFixture() {
    ...
    const ConnetVaultOfStHOPE = await ethers.getContractFactory(
      "ConnectVaultOfStHOPE"
    );
    const connetVaultOfStHOPE = await upgrades.deployProxy(
      ConnetVaultOfStHOPE,
      [
        permit2.address,
        stakingHope.address,
    ...
    await mockConnet.setVault(connetVaultOfStHOPE.address);
  ...
  describe("Initialize check", async function () {
    it("Initialize check", async function () {
      const { owner, alice, bob, connetVaultOfStHOPE, adminRole } =
        await loadFixture(deployOneYearLockFixture);
      expect(await connetVaultOfStHOPE.owner()).to.be.equal(owner.address);
      expect(await connetVaultOfStHOPE.hasRole(adminRole, alice.address)).to
        .true;
    });
  });
  ...
  describe("transferLTRewards", async function () {
    it("should revert right error when caller is not connnet", async function () {
      const { owner, alice, bob, connetVaultOfStHOPE, adminRole } =
        await loadFixture(deployOneYearLockFixture);
      await expect(
        connetVaultOfStHOPE.transferLTRewards(
          bob.address,
          BigNumber.from("100")
        )
    ...
    it("transferLTRewards", async function () {
      ...
      await hopeToken
        .connect(alice)
        .approve(stakingHope.address, stakingAmount);
      await stakingHope
        .connect(alice)
        .staking(
          stakingAmount,
      ...
      await stakingHope
        .connect(alice)
        .approve(connetVaultOfStHOPE.address, amount);
      await connetVaultOfStHOPE
        .connect(alice)
        .deposit(amount, ethers.constants.Zero, ethers.constants.Zero, "0x");

      expect(await lt.balanceOf(mockConnet.address)).to.be.equal(0);
      await time.increase(86400 * 10);
      await mockConnet.setLt(lt.address);
      await mockConnet.transferLTRewards(
        bob.address,
        ethers.utils.parseEther("1000")
      );
      expect(await lt.balanceOf(bob.address)).to.be.equal(
        ethers.utils.parseEther("1000")
      );
    });
  });
