...
describe("ConnetVault", function () {
    ...
    async function deployOneYearLockFixture() {
        ...
        const connetVault = await upgrades.deployProxy(ConnetVault, [permit2.address, hopeToken.address, mockConnet.address, alice.address, owner.address]);
        await connetVault.deployed();

        await mockConnet.setValut(connetVault.address);

        let adminRole = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("withraw_Admin_Role"));

        return { owner, alice, bob, connetVault, permit2, hopeToken, mockConnet, admin, adminRole };
    }

    describe("Initialize check", async function () {

        it("should revert right error when already initialized", async function () {
            const { owner, alice, bob, connetVault, permit2, hopeToken, mockConnet } = await loadFixture(deployOneYearLockFixture);

            await expect(connetVault.initialize(permit2.address, hopeToken.address, mockConnet.address, alice.address, owner.address)).to.be.revertedWith("Initializable: contract is already initialized");
        });

        it("Initialize check", async function () {
            const { owner, alice, bob, connetVault, adminRole } = await loadFixture(deployOneYearLockFixture);
    ...
    describe("Owner authority check", async function () {
        ...
        it("should revert right error when caller is not the owner", async function () {
            ...
            await expect(connetVault.connect(alice).changeConnet(permit2.address)).to.be.revertedWith("Ownable: caller is not the owner");
        });

        it("changeConnet", async function () {
            const { owner, alice, bob, connetVault } = await loadFixture(deployOneYearLockFixture);

            await expect(await connetVault.changeConnet(bob.address)).to.emit(connetVault, "ChangeConnet").withArgs(bob.address);
        });

        it("grantRole", async function () {
            const { owner, bob, connetVault, adminRole } = await loadFixture(deployOneYearLockFixture);
    ...
    describe("deposit", async function () {
        ...
        it("should revert right error when pause", async function () {
            ...
            await expect(connetVault.deposit(amount, ethers.constants.Zero, ethers.constants.Zero, ethers.constants.HashZero)).to.be.revertedWith("Pausable: paused");
        });
        ...
        it("should revert right error when amount zero", async function () {
            ...
            await expect(connetVault.deposit(amount, ethers.constants.Zero, ethers.constants.Zero, ethers.constants.HashZero)).to.be.revertedWith("INVALID_ZERO_AMOUNT");
        });
        ...
        it("deposit with transferFrom", async function () {
            ...
            await expect(await connetVault.deposit(amount, ethers.constants.Zero, ethers.constants.Zero, "0x")).to.emit(connetVault, "Deposit").withArgs(owner.address, amount);
            expect(await hopeToken.balanceOf(owner.address)).to.be.equal(beforeBalance.sub(amount));
            expect(await hopeToken.balanceOf(connetVault.address)).to.be.equal(amount);
            expect(await connetVault.balanceOf(mockConnet.address)).to.be.equal(amount);
        });
        ...
        it("deposit with permit2", async function () {
            ...
            const sig = await PermitSigHelper.signature(owner, hopeToken.address, permit2.address, connetVault.address, amount, NONCE, DEADLINE);

            let beforeBalance = await hopeToken.balanceOf(owner.address);
            await expect(await connetVault.deposit(amount, NONCE, DEADLINE, sig)).to.emit(connetVault, "Deposit").withArgs(owner.address, amount);
            expect(await hopeToken.balanceOf(owner.address)).to.be.equal(beforeBalance.sub(amount));
            expect(await hopeToken.balanceOf(connetVault.address)).to.be.equal(amount);
            expect(await connetVault.balanceOf(mockConnet.address)).to.be.equal(amount);
        });
    ...
    describe("withdraw", async function () {
        ...
        it("should revert right error when caller is not withdrawAdmin", async function () {
            ...
            let amount = ethers.utils.parseEther("10");
            await expect(connetVault.connect(bob).withdraw(bob.address, amount)).to.be.revertedWith("AccessControl: account 0x3c44cdddb6a900fa2b585dd299e03d12fa4293bc is missing role 0x90ceb4755143c770f991c04212ab04997955846b6c85bdbef5da1c556453c09e");
        });
        ...
        it("should revert right error when  insufficient balance", async function () {
            ...
            let amount = ethers.utils.parseEther("10");
            await expect(connetVault.connect(alice).withdraw(bob.address, amount)).to.be.revertedWith("insufficient balance");
        });
        ...
        it("should revert right error when amount is zero", async function () {
            ...
            let amount = ethers.utils.parseEther("0");
            await expect(connetVault.connect(alice).withdraw(bob.address, amount)).to.be.revertedWith("INVALID_ZERO_AMOUNT");
        });

        it("withdraw success", async function () {
            const { owner, alice, bob, connetVault, hopeToken, mockConnet } = await loadFixture(deployOneYearLockFixture);

            let amount = ethers.utils.parseEther("10");
            await hopeToken.approve(connetVault.address, amount);
            await connetVault.deposit(amount, ethers.constants.Zero, ethers.constants.Zero, "0x")
            ...
            expect(await hopeToken.balanceOf(bob.address)).to.be.equal(withdawaAmount);
            expect(await hopeToken.balanceOf(bob.address)).to.be.equal(withdawaAmount);
        });


        it("withdraw success v2", async function () {
            const { owner, alice, bob, connetVault, hopeToken, mockConnet, adminRole } = await loadFixture(deployOneYearLockFixture);
